...
task(
  ...
  `Deploys GameNFT.sol and DestinationMinter.sol smart contracts`
)
  .addOptionalParam(
    `router`,
  ...
  .setAction(
    async (taskArguments: TaskArguments, hre: HardhatRuntimeEnvironment) => {
      ...
      console.log(
        `ℹ️  Attempting to deploy GameNFT smart contract on the ${hre.network.name} blockchain using ${deployer.address} address`
      );
      spinner.start();

      const GameNFTFactory: GameNFT__factory =
        (await hre.ethers.getContractFactory("GameNFT")) as GameNFT__factory;
      const GameNFT: GameNFT = await GameNFTFactory.deploy(wallet.address);
      await GameNFT.deployed();

      spinner.stop();
      console.log(
        `✅ GameNFT contract deployed at address ${GameNFT.address} on the ${hre.network.name} blockchain`
      );
      ...
      const destinationMinter: DestinationMinter =
        await destinationMinterFactory.deploy(routerAddress, GameNFT.address);
      ...
      const tx = await GameNFT.transferOwnership(destinationMinter.address);
      await tx.wait();

      spinner.stop();
      console.log(
        `✅ DestinationMinter can now mint GameNFTs. Transaction hash: ${tx.hash}`
      );
    }
  );
